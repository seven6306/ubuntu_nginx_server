...
read -p "Setup nginx server will take 30-60 minutes, Are you sure? [y/N]: " ans
case $ans in
...
printf "=============================================================================\n\n"
sed -i.bak '/# Make site accessible from/ a \ \ \ \ \ \ \ \ ssl_ciphers "EECDH+AESGCM:EDH+AESGCM:AES256+EECDH:AES256+EDH";' /etc/nginx/sites-enabled/default
for each_line in 'ssl_protocols TLSv1 TLSv1.1 TLSv1.2;' 'ssl_certificate_key /etc/nginx/ssl/nginx.key;' 'ssl_certificate /etc/nginx/ssl/nginx.crt;' 'ssl_dhparam /etc/nginx/ssl/dhparam.pem;' 'ssl_prefer_server_ciphers on;' 'ssl on;' 'listen 443 ssl;' '#SSL Part' ''
