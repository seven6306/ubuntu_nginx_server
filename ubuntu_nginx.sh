...
case $? in
	0) rm -f /etc/init.d/request.tmp;;
	*) printf "${RED}ERROR: Permission denied, try \'sudo\' to execute the script.${NC}\n" && exit 1;;
esac
[ `dpkg -l | awk '{print $2}' | grep -co nginx >> /dev/null 2>&1` -ne 0 ] && printf "${RED}ERROR: Service nginx is already installed.${NC}\n" && exit 1
printf "${PURPLE}Start installing nginx...${NC}\n"
apt-get update
apt-get install nginx -y
[ ! -d /etc/nginx/ssl ] && sudo mkdir -p /etc/nginx/ssl || rm -rf /etc/nginx/ssl/*
printf "${PURPLE}Generate ssl certification:${NC}\n"
